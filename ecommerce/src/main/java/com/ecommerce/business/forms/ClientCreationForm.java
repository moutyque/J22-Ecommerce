...
public class ClientCreationForm {
    ...
    private void validationAdress( String adresse ) throws Exception {
        if ( adresse == null || adresse.length() < 10 ) {
            throw new Exception( "L'adressse d'utilisateur doit contenir au moins 10 caractères." );
        }
    }

    private void validationTelephone( String telephone ) throws Exception {
        if ( telephone == null || telephone.length() < 10 ) {
            throw new Exception( "Le téléphone de l'utilisateur doit contenir au moins 4 caractères." );
        }
    }
